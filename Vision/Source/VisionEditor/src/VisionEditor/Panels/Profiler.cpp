...
VisionEditor::Panels::Profiler::Profiler
(
...
void VisionEditor::Panels::Profiler::Update(float p_deltaTime)
{
	m_timer += p_deltaTime;
...
void VisionEditor::Panels::Profiler::Enable(bool p_value, bool p_disableLog)
{
	if (p_value)
	{
		if (!p_disableLog)
...
VisionUI::Types::Color VisionEditor::Panels::Profiler::CalculateActionColor(double p_percentage) const
{
...
std::string VisionEditor::Panels::Profiler::GenerateActionString(VisionAnalytics::Profiling::ProfilerReport::Action & p_action)
{
	std::string result;
