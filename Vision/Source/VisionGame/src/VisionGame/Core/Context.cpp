...
VisionGame::Core::Context::Context() :
	...
	sceneManager(projectAssetsPath)
{
	...
	driver = std::make_unique<VisionRendering::Context::Driver>(VisionRendering::Settings::DriverSettings{ false });
	renderer = std::make_unique<VisionCore::ECS::Renderer>(*driver);

	renderer->SetCapability(VisionRendering::Settings::ERenderingCapability::MULTISAMPLE, projectSettings.Get<bool>("multisampling"));
	...
	/* Physics engine */
	physicsEngine = std::make_unique<VisionPhysics::Core::PhysicsEngine>(VisionPhysics::Settings::PhysicsSettings{ {0.0f, projectSettings.Get<float>("gravity"), 0.0f } });
