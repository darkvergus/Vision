...
VisionGame::Core::Game::Game(Context & p_context) :
	m_context(p_context),
...
VisionGame::Core::Game::~Game()
{
	m_context.sceneManager.UnloadCurrentScene();
}

void VisionGame::Core::Game::PreUpdate()
{
	#ifdef _DEBUG
...
void VisionGame::Core::Game::Update(float p_deltaTime)
{
	m_elapsed += p_deltaTime;

	m_context.engineUBO->SetSubData(m_elapsed, 3 * sizeof(VisionMaths::FMatrix4) + sizeof(VisionMaths::FVector3));
	...
	if  (m_context.inputManager->IsKeyPressed(VisionWindowing::Inputs::EKey::KEY_F12))
		m_showDebugInformation = !m_showDebugInformation;

	#ifdef _DEBUG
	if (m_context.inputManager->IsKeyPressed(VisionWindowing::Inputs::EKey::KEY_R))
		VisionRendering::Resources::Loaders::ShaderLoader::Recompile(*m_context.shaderManager[":Shaders\\Standard.glsl"], "Data\\Engine\\Shaders\\Standard.glsl");
	#endif
...
void VisionGame::Core::Game::PostUpdate()
{
	#ifdef _DEBUG
