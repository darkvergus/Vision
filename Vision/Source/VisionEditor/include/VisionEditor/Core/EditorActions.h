...
#define EDITOR_EXEC(action)					VisionCore::Global::ServiceLocator::Get<VisionEditor::Core::EditorActions>().action
#define EDITOR_BIND(method, ...)			std::bind(&VisionEditor::Core::EditorActions::method, &VisionCore::Global::ServiceLocator::Get<VisionEditor::Core::EditorActions>(), ##__VA_ARGS__)
#define EDITOR_EVENT(target)				VisionCore::Global::ServiceLocator::Get<VisionEditor::Core::EditorActions>().target
#define EDITOR_CONTEXT(instance)			VisionCore::Global::ServiceLocator::Get<VisionEditor::Core::EditorActions>().GetContext().instance
#define EDITOR_RENDERER()					VisionCore::Global::ServiceLocator::Get<VisionEditor::Core::EditorActions>().GetRenderer()
#define EDITOR_PANEL(type, id)				VisionCore::Global::ServiceLocator::Get<VisionEditor::Core::EditorActions>().GetPanelsManager().GetPanelAs<type>(id)
...
namespace VisionEditor::Core
{
	...
	*/
	...
	public:
		...
		* @param p_parent
		*/
		template<typename T> VisionCore::ECS::Actor& CreateMonoComponentActor(bool p_focusOnCreation = true, VisionCore::ECS::Actor* p_parent = nullptr);

		/**
		* Calculate the position where to spawn the actor using the current camera position and forward
		* @param p_distanceToCamera
		*/
		VisionMaths::FVector3 CalculateActorSpawnPoint(float p_distanceToCamera);
        ...
        * @param p_name
		*/
		VisionCore::ECS::Actor&	CreateEmptyActor(bool p_focusOnCreation = true, VisionCore::ECS::Actor* p_parent = nullptr, const std::string& p_name = "");
        ...
        * @param p_name
		*/
		VisionCore::ECS::Actor&	CreateActorWithModel(const std::string& p_path, bool p_focusOnCreation = true, VisionCore::ECS::Actor* p_parent = nullptr, const std::string& p_name = "");
		...
		* @param p_parent
		*/
		bool DestroyActor(VisionCore::ECS::Actor& p_actor);
		...
		* @param bool
		*/
		void DuplicateActor(VisionCore::ECS::Actor& p_toDuplicate, VisionCore::ECS::Actor* p_forcedParent = nullptr, bool p_focus = true);
		#pragma endregion
		...
		* @param p_target
		*/
		void SelectActor(VisionCore::ECS::Actor& p_target);
		...
		*/
		VisionCore::ECS::Actor&		GetSelectedActor() const;
		...
		* @param p_fileType
		*/
		void PropagateFileRenameThroughSavedFilesOfType(const std::string& p_previousName, const std::string& p_newName, VisionTools::Utils::PathParser::EFileType p_fileType);
		#pragma endregion
	...
	public:
		...
		VisionTools::Eventing::Event<EEditorMode> EditorModeChangedEvent;
		VisionTools::Eventing::Event<> PlayEvent;
...
#include "VisionEditor/Core/EditorActions.inl"