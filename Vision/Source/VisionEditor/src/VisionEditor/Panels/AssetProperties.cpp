...
#include <VisionTools/Utils/SizeConverter.h>
...
#include <VisionUI/Widgets/Layout/NewLine.h>
...
VisionEditor::Panels::AssetProperties::AssetProperties
(
	...
	PanelWindow(p_title, p_opened, p_windowSettings)
{
    ...
    m_headerSeparator = &CreateWidget<VisionUI::Widgets::Visual::Separator>();
    m_headerSeparator->enabled = false;

    CreateAssetSelector();

    m_settings = &CreateWidget<VisionUI::Widgets::Layout::GroupCollapsable>("Settings");
	m_settingsColumns = &m_settings->CreateWidget<VisionUI::Widgets::Layout::Columns<2>>();
	m_settingsColumns->widths[0] = 150;

    m_info = &CreateWidget<VisionUI::Widgets::Layout::GroupCollapsable>("Info");
    m_infoColumns = &m_info->CreateWidget<VisionUI::Widgets::Layout::Columns<2>>();
    m_infoColumns->widths[0] = 150;

    m_settings->enabled = m_info->enabled = false;
}

void VisionEditor::Panels::AssetProperties::SetTarget(const std::string& p_path)
{
...
void VisionEditor::Panels::AssetProperties::Refresh()
{
    m_metadata.reset(new VisionTools::Filesystem::IniFile(EDITOR_EXEC(GetRealPath(m_resource)) + ".meta"));
    ...
    switch (VisionTools::Utils::PathParser::GetFileType(m_resource))
    {
...
void VisionEditor::Panels::AssetProperties::Preview()
{
	auto& assetView = EDITOR_PANEL(VisionEditor::Panels::AssetView, "Asset View");

	const auto fileType = VisionTools::Utils::PathParser::GetFileType(m_resource);

	if (fileType == VisionTools::Utils::PathParser::EFileType::MODEL)
	{
		if (auto resource = SERVICE(VisionCore::ResourceManagement::ModelManager).GetResource(m_resource))
		{
			assetView.SetResource(resource);
		}
	}
	else if (fileType == VisionTools::Utils::PathParser::EFileType::TEXTURE)
	{
		if (auto resource = SERVICE(VisionCore::ResourceManagement::TextureManager).GetResource(m_resource))
		{
			assetView.SetResource(resource);
		}
	}
...
void VisionEditor::Panels::AssetProperties::CreateHeaderButtons()
{
	m_applyButton = &CreateWidget<VisionUI::Widgets::Buttons::Button>("Apply");
	...
	m_revertButton = &CreateWidget<VisionUI::Widgets::Buttons::Button>("Revert");
	...
	m_previewButton = &CreateWidget<VisionUI::Widgets::Buttons::Button>("Preview");
	...
	m_resetButton = &CreateWidget<VisionUI::Widgets::Buttons::Button>("Reset to default");
    ...
    m_headerLineBreak = &CreateWidget<VisionUI::Widgets::Layout::NewLine>();
    m_headerLineBreak->enabled = false;
}

void VisionEditor::Panels::AssetProperties::CreateAssetSelector()
{
    auto& columns = CreateWidget<VisionUI::Widgets::Layout::Columns<2>>();
    columns.widths[0] = 150;
    m_assetSelector = &VisionCore::Helpers::GUIDrawer::DrawAsset(columns, "Target", m_resource, &m_targetChanged);
}

void VisionEditor::Panels::AssetProperties::CreateSettings()
{
...
void VisionEditor::Panels::AssetProperties::CreateInfo()
{
    ...
    if (std::filesystem::exists(realPath))
    {
        ...
        VisionCore::Helpers::GUIDrawer::CreateTitle(*m_infoColumns, "Path");
        m_infoColumns->CreateWidget<VisionUI::Widgets::Texts::Text>(realPath);

        VisionCore::Helpers::GUIDrawer::CreateTitle(*m_infoColumns, "Size");
        const auto [size, unit] = VisionTools::Utils::SizeConverter::ConvertToOptimalUnit(static_cast<float>(std::filesystem::file_size(realPath)), VisionTools::Utils::SizeConverter::ESizeUnit::BYTE);
        m_infoColumns->CreateWidget<VisionUI::Widgets::Texts::Text>(std::to_string(size) + " " + VisionTools::Utils::SizeConverter::UnitToString(unit));

        VisionCore::Helpers::GUIDrawer::CreateTitle(*m_infoColumns, "Metadata");
        m_infoColumns->CreateWidget<VisionUI::Widgets::Texts::Text>(std::filesystem::exists(realPath + ".meta") ? "Yes" : "No");
    }
    else
    {
...
#define MODEL_FLAG_ENTRY(setting) VisionCore::Helpers::GUIDrawer::DrawBoolean(*m_settingsColumns, setting, [&]() { return m_metadata->Get<bool>(setting); }, [&](bool value) { m_metadata->Set<bool>(setting, value); })

void VisionEditor::Panels::AssetProperties::CreateModelSettings()
{
...
void VisionEditor::Panels::AssetProperties::CreateTextureSettings()
{
	m_metadata->Add("MIN_FILTER", static_cast<int>(VisionRendering::Settings::ETextureFilteringMode::LINEAR_MIPMAP_LINEAR));
	m_metadata->Add("MAG_FILTER", static_cast<int>(VisionRendering::Settings::ETextureFilteringMode::LINEAR));
	...
	VisionCore::Helpers::GUIDrawer::CreateTitle(*m_settingsColumns, "MIN_FILTER");
	auto& minFilter = m_settingsColumns->CreateWidget<VisionUI::Widgets::Selection::ComboBox>(m_metadata->Get<int>("MIN_FILTER"));
	...
	VisionCore::Helpers::GUIDrawer::CreateTitle(*m_settingsColumns, "MAG_FILTER");
	auto& magFilter = m_settingsColumns->CreateWidget<VisionUI::Widgets::Selection::ComboBox>(m_metadata->Get<int>("MAG_FILTER"));
	...
	VisionCore::Helpers::GUIDrawer::DrawBoolean(*m_settingsColumns, "ENABLE_MIPMAPPING", [&]() { return m_metadata->Get<bool>("ENABLE_MIPMAPPING"); }, [&](bool value) { m_metadata->Set<bool>("ENABLE_MIPMAPPING", value); });
}

void VisionEditor::Panels::AssetProperties::Apply()
{
	m_metadata->Rewrite();
