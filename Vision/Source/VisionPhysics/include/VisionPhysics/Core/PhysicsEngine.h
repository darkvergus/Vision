...
#include "VisionPhysics/Settings/PhysicsSettings.h"
#include "VisionPhysics/Entities/RaycastHit.h"

namespace VisionPhysics::Core
{
	/**
	...
	*/
	...
	public:
		...
		*/
		 ...
		 * @param p_end
		 */
		std::optional<Entities::RaycastHit> Raycast(VisionMaths::FVector3 p_origin, VisionMaths::FVector3 p_direction, float p_distance);

		/**
		* Defines the world gravity to apply
		* @param p_gravity
		*/
		void SetGravity(const VisionMaths::FVector3& p_gravity);

		/**
		* Returns the current world gravity
		*/
		VisionMaths::FVector3 GetGravity() const;
	...
	private:
		...
		void Consider(VisionPhysics::Entities::PhysicalObject& p_toConsider);
		void Unconsider(VisionPhysics::Entities::PhysicalObject& p_toUnconsider);
