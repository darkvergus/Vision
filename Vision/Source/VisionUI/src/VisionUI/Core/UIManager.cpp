...
VisionUI::Core::UIManager::UIManager(GLFWwindow* p_glfwWindow, Styling::EStyle p_style, const std::string& p_glslVersion)
{
	ImGui::CreateContext();
...
VisionUI::Core::UIManager::~UIManager()
{
...
void VisionUI::Core::UIManager::ApplyStyle(Styling::EStyle p_style)
{
	ImGuiStyle* style = &ImGui::GetStyle();

	switch (p_style)
	{
	case VisionUI::Styling::EStyle::IM_CLASSIC_STYLE:	ImGui::StyleColorsClassic();	break;
	case VisionUI::Styling::EStyle::IM_DARK_STYLE:		ImGui::StyleColorsDark();		break;
	case VisionUI::Styling::EStyle::IM_LIGHT_STYLE:		ImGui::StyleColorsLight();		break;
	}

	if (p_style == VisionUI::Styling::EStyle::DUNE_DARK)
	{
    ...
    else if (p_style == VisionUI::Styling::EStyle::ALTERNATIVE_DARK)
    {
        style->WindowPadding = ImVec2(15, 15);
...
bool VisionUI::Core::UIManager::LoadFont(const std::string& p_id, const std::string & p_path, float p_fontSize)
{
...
bool VisionUI::Core::UIManager::UnloadFont(const std::string & p_id)
{
	if (m_fonts.find(p_id) != m_fonts.end())
	{
		m_fonts.erase(p_id);
...
bool VisionUI::Core::UIManager::UseFont(const std::string & p_id)
{
...
void VisionUI::Core::UIManager::UseDefaultFont()
{
	ImGui::GetIO().FontDefault = nullptr;
}

void VisionUI::Core::UIManager::EnableEditorLayoutSave(bool p_value)
{
	if (p_value)
...
bool VisionUI::Core::UIManager::IsEditorLayoutSaveEnabled() const
{
	return ImGui::GetIO().IniFilename != nullptr;
}

void VisionUI::Core::UIManager::SetEditorLayoutSaveFilename(const std::string & p_filename)
{
...
void VisionUI::Core::UIManager::SetEditorLayoutAutosaveFrequency(float p_frequency)
{
	ImGui::GetIO().IniSavingRate = p_frequency;
}

float VisionUI::Core::UIManager::GetEditorLayoutAutosaveFrequency(float p_frequeny)
{
	return ImGui::GetIO().IniSavingRate;
}

void VisionUI::Core::UIManager::EnableDocking(bool p_value)
{
...
void VisionUI::Core::UIManager::ResetLayout(const std::string& p_config) const
{
    ImGui::LoadIniSettingsFromDisk(p_config.c_str());
}

bool VisionUI::Core::UIManager::IsDockingEnabled() const
{
	return m_dockingState;
}

void VisionUI::Core::UIManager::SetCanvas(Modules::Canvas& p_canvas)
{
	RemoveCanvas();
...
void VisionUI::Core::UIManager::RemoveCanvas()
{
	m_currentCanvas = nullptr;
}

void VisionUI::Core::UIManager::Render()
{
	if (m_currentCanvas)
	{
...
void VisionUI::Core::UIManager::PushCurrentFont()
{

}

void VisionUI::Core::UIManager::PopCurrentFont()
{
