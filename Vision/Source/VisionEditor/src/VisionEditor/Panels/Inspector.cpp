...
#include <VisionUI/Widgets/InputFields/InputFloat.h>
...
VisionEditor::Panels::Inspector::Inspector
(
...
{
	m_inspectorHeader = &CreateWidget<VisionUI::Widgets::Layout::Group>();
	m_inspectorHeader->enabled = false;
	m_actorInfo = &CreateWidget<VisionUI::Widgets::Layout::Group>();

	auto& headerColumns = m_inspectorHeader->CreateWidget<VisionUI::Widgets::Layout::Columns<2>>();
	...
	VisionCore::Helpers::GUIDrawer::DrawString(headerColumns, "Name", nameGatherer, nameProvider);
	...
	VisionCore::Helpers::GUIDrawer::DrawString(headerColumns, "Tag", tagGatherer, tagProvider);
	...
	VisionCore::Helpers::GUIDrawer::DrawBoolean(headerColumns, "Active", activeGatherer, activeProvider);

	/* Component select + button */
	{
		auto& componentSelectorWidget = m_inspectorHeader->CreateWidget<VisionUI::Widgets::Selection::ComboBox>(0);
		...
		auto& addComponentButton = m_inspectorHeader->CreateWidget<VisionUI::Widgets::Buttons::Button>("Add Component", VisionMaths::FVector2{ 100.f, 0 });
		addComponentButton.idleBackgroundColor = VisionUI::Types::Color{ 0.7f, 0.5f, 0.f };
		addComponentButton.textColor = VisionUI::Types::Color::White;
		...
		componentSelectorWidget.ValueChangedEvent += [this, &addComponentButton](int p_value)
		{
			auto defineButtonsStates = [&addComponentButton](bool p_componentExists)
			{
				...
				addComponentButton.idleBackgroundColor = !p_componentExists ? VisionUI::Types::Color{ 0.7f, 0.5f, 0.f } : VisionUI::Types::Color{ 0.1f, 0.1f, 0.1f };
			};
	...
	{
		m_scriptSelectorWidget = &m_inspectorHeader->CreateWidget<VisionUI::Widgets::InputFields::InputText>("");
        m_scriptSelectorWidget->lineBreak = false;
		auto& ddTarget = m_scriptSelectorWidget->AddPlugin<VisionUI::Plugins::DDTarget<std::pair<std::string, Layout::Group*>>>("File");
		
		auto& addScriptButton = m_inspectorHeader->CreateWidget<VisionUI::Widgets::Buttons::Button>("Add Script", VisionMaths::FVector2{ 100.f, 0 });
		addScriptButton.idleBackgroundColor = VisionUI::Types::Color{ 0.7f, 0.5f, 0.f };
		addScriptButton.textColor = VisionUI::Types::Color::White;
        ...
        const auto updateAddScriptButton = [&addScriptButton, this](const std::string& p_script)
        {
            ...
            addScriptButton.idleBackgroundColor = isScriptValid ? VisionUI::Types::Color{ 0.7f, 0.5f, 0.f } : VisionUI::Types::Color{ 0.1f, 0.1f, 0.1f };
        };
	...
	m_inspectorHeader->CreateWidget<VisionUI::Widgets::Visual::Separator>();

	m_destroyedListener = VisionCore::ECS::Actor::DestroyedEvent += [this](VisionCore::ECS::Actor& p_destroyed)
	{ 
...
VisionEditor::Panels::Inspector::~Inspector()
{
	VisionCore::ECS::Actor::DestroyedEvent -= m_destroyedListener;

	UnFocus();
}

void VisionEditor::Panels::Inspector::FocusActor(VisionCore::ECS::Actor& p_target)
{
	if (m_targetActor)
		UnFocus();
...
void VisionEditor::Panels::Inspector::UnFocus()
{
	if (m_targetActor)
	{
...
void VisionEditor::Panels::Inspector::SoftUnFocus()
{
    if (m_targetActor)
    {
...
VisionCore::ECS::Actor * VisionEditor::Panels::Inspector::GetTargetActor() const
{
	return m_targetActor;
}

void VisionEditor::Panels::Inspector::CreateActorInspector(VisionCore::ECS::Actor& p_target)
{
	std::map<std::string, VisionCore::ECS::Components::AComponent*> components;

	for (auto component : p_target.GetComponents())
		if (component->GetName() != "Transform")
			components[component->GetName()] = component.get();

	auto transform = p_target.GetComponent<VisionCore::ECS::Components::CTransform>();
	if (transform)
...
void VisionEditor::Panels::Inspector::DrawComponent(VisionCore::ECS::Components::AComponent& p_component)
{
	if (auto inspectorItem = dynamic_cast<VisionCore::API::IInspectorItem*>(&p_component); inspectorItem)
	{
		auto& header = m_actorInfo->CreateWidget<VisionUI::Widgets::Layout::GroupCollapsable>(p_component.GetName());
		header.closable = !dynamic_cast<VisionCore::ECS::Components::CTransform*>(&p_component);
		header.CloseEvent += [this, &header, &p_component]
		{ 
			if (p_component.owner.RemoveComponent(p_component))
				m_componentSelectorWidget->ValueChangedEvent.Invoke(m_componentSelectorWidget->currentChoice);
		};
		auto& columns = header.CreateWidget<VisionUI::Widgets::Layout::Columns<2>>();
		columns.widths[0] = 200;
		inspectorItem->OnInspector(columns);
	}
}

void VisionEditor::Panels::Inspector::DrawBehaviour(VisionCore::ECS::Components::Behaviour & p_behaviour)
{
	if (auto inspectorItem = dynamic_cast<VisionCore::API::IInspectorItem*>(&p_behaviour); inspectorItem)
	{
		auto& header = m_actorInfo->CreateWidget<VisionUI::Widgets::Layout::GroupCollapsable>(p_behaviour.name);
...
void VisionEditor::Panels::Inspector::Refresh()
{
	if (m_targetActor)
	{
		m_actorInfo->RemoveAllWidgets();
