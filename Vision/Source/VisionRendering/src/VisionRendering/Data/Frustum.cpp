...
void VisionRendering::Data::Frustum::CalculateFrustum(const VisionMaths::FMatrix4& p_viewProjection)
{
	auto columnMajorViewProjection = VisionMaths::FMatrix4::Transpose(p_viewProjection);
...
bool VisionRendering::Data::Frustum::PointInFrustum(float x, float y, float z) const
{
...
bool VisionRendering::Data::Frustum::SphereInFrustum(float x, float y, float z, float radius) const
{
...
bool VisionRendering::Data::Frustum::CubeInFrustum(float x, float y, float z, float size) const
{
...
bool VisionRendering::Data::Frustum::BoundingSphereInFrustum(const VisionRendering::Geometry::BoundingSphere& p_boundingSphere, const VisionMaths::FTransform& p_transform) const
{
	...
	auto sphereOffset = VisionMaths::FQuaternion::RotatePoint(p_boundingSphere.position, rotation) * maxScale;

	VisionMaths::FVector3 worldCenter = position + sphereOffset;

	return SphereInFrustum(worldCenter.x, worldCenter.y, worldCenter.z, scaledRadius);
}

std::array<float, 4> VisionRendering::Data::Frustum::GetNearPlane() const
{
	return { m_frustum[FRONT][0], m_frustum[FRONT][1], m_frustum[FRONT][2], m_frustum[FRONT][3] };
}

std::array<float, 4> VisionRendering::Data::Frustum::GetFarPlane() const
{
