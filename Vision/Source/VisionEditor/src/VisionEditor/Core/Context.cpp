...
using namespace VisionCore::Global;
using namespace VisionCore::ResourceManagement;

VisionEditor::Core::Context::Context(const std::string& p_projectPath, const std::string& p_projectName) :
	...
	projectSettings(projectFilePath)
{
	...
	windowSettings.title = "Vision Editor";
	windowSettings.width = 1280;
	windowSettings.height = 720;
	windowSettings.maximized = true;

	/* Window creation */
	device = std::make_unique<VisionWindowing::Context::Device>(deviceSettings);
	window = std::make_unique<VisionWindowing::Window>(*device, windowSettings);
	...
	inputManager = std::make_unique<VisionWindowing::Inputs::InputManager>(*window);
	window->MakeCurrentContext();

	device->SetVsync(true);

	/* Graphics context creation */
	driver = std::make_unique<VisionRendering::Context::Driver>(VisionRendering::Settings::DriverSettings{ true });
	renderer = std::make_unique<VisionCore::ECS::Renderer>(*driver);
	renderer->SetCapability(VisionRendering::Settings::ERenderingCapability::MULTISAMPLE, true);
	shapeDrawer = std::make_unique<VisionRendering::Core::ShapeDrawer>(*renderer);

	std::filesystem::create_directories(std::string(getenv("APPDATA")) + "\\Vision\\VisionEditor\\");

	uiManager = std::make_unique<VisionUI::Core::UIManager>(window->GetGlfwWindow(), VisionUI::Styling::EStyle::ALTERNATIVE_DARK);
	...
	uiManager->SetEditorLayoutSaveFilename(std::string(getenv("APPDATA")) + "\\Vision\\VisionEditor\\layout.ini");
	uiManager->SetEditorLayoutAutosaveFrequency(60.0f);
	uiManager->EnableEditorLayoutSave(true);
	uiManager->EnableDocking(true);

	if (!std::filesystem::exists(std::string(getenv("APPDATA")) + "\\Vision\\VisionEditor\\layout.ini"))
		uiManager->ResetLayout("Config\\layout.ini");

	/* Audio */
	audioEngine = std::make_unique<VisionAudio::Core::AudioEngine>(projectAssetsPath);
	audioPlayer = std::make_unique<VisionAudio::Core::AudioPlayer>(*audioEngine);

	/* Editor resources */
	editorResources = std::make_unique<VisionEditor::Core::EditorResources>(editorAssetsPath);

	/* Physics engine */
	physicsEngine = std::make_unique<VisionPhysics::Core::PhysicsEngine>(VisionPhysics::Settings::PhysicsSettings{ {0.0f, -9.81f, 0.0f } });

	/* Service Locator providing */
	ServiceLocator::Provide<VisionPhysics::Core::PhysicsEngine>(*physicsEngine);
	...
	ServiceLocator::Provide<VisionWindowing::Inputs::InputManager>(*inputManager);
	ServiceLocator::Provide<VisionWindowing::Window>(*window);
	ServiceLocator::Provide<VisionCore::SceneSystem::SceneManager>(sceneManager);
	ServiceLocator::Provide<VisionAudio::Core::AudioEngine>(*audioEngine);
	ServiceLocator::Provide<VisionAudio::Core::AudioPlayer>(*audioPlayer);

	/* Scripting */
	scriptInterpreter = std::make_unique<VisionCore::Scripting::ScriptInterpreter>(projectScriptsPath);

	engineUBO = std::make_unique<VisionRendering::Buffers::UniformBuffer>
	(
		/* UBO Data Layout */
		...
		sizeof(VisionMaths::FMatrix4),
		0, 0,
		VisionRendering::Buffers::EAccessSpecifier::STREAM_DRAW
	);

	lightSSBO			= std::make_unique<VisionRendering::Buffers::ShaderStorageBuffer>(VisionRendering::Buffers::EAccessSpecifier::STREAM_DRAW);
	simulatedLightSSBO	= std::make_unique<VisionRendering::Buffers::ShaderStorageBuffer>(VisionRendering::Buffers::EAccessSpecifier::STREAM_DRAW); // Used in Asset View

	std::vector<VisionMaths::FMatrix4> simulatedLights;

	VisionMaths::FTransform simulatedLightTransform;
	simulatedLightTransform.SetLocalRotation(VisionMaths::FQuaternion({ 45.f, 180.f, 10.f }));

	VisionRendering::Entities::Light simulatedDirectionalLight(simulatedLightTransform, VisionRendering::Entities::Light::Type::DIRECTIONAL);
	simulatedDirectionalLight.color = { 1.f, 1.f, 1.f };
	simulatedDirectionalLight.intensity = 1.f;

	VisionRendering::Entities::Light simulatedAmbientLight(simulatedLightTransform, VisionRendering::Entities::Light::Type::AMBIENT_SPHERE);
	...
	simulatedLightSSBO->SendBlocks<VisionMaths::FMatrix4>(simulatedLights.data(), simulatedLights.size() * sizeof(VisionMaths::FMatrix4));

	ApplyProjectSettings();
}

VisionEditor::Core::Context::~Context()
{
...
void VisionEditor::Core::Context::ResetProjectSettings()
{
...
bool VisionEditor::Core::Context::IsProjectSettingsIntegrityVerified()
{
	return
...
void VisionEditor::Core::Context::ApplyProjectSettings()
{
