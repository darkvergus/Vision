...
#include <VisionUI/Widgets/Sliders/SliderInt.h>
#include <VisionUI/Widgets/Sliders/SliderFloat.h>
...
VisionEditor::Panels::MenuBar::MenuBar()
{
	CreateFileMenu();
	CreateBuildMenu();
	CreateWindowMenu();
...
void VisionEditor::Panels::MenuBar::HandleShortcuts(float p_deltaTime)
{
	auto& inputManager = *EDITOR_CONTEXT(inputManager);

	if (inputManager.GetKeyState(VisionWindowing::Inputs::EKey::KEY_LEFT_CONTROL) == VisionWindowing::Inputs::EKeyState::KEY_DOWN)
	{
		if (inputManager.IsKeyPressed(VisionWindowing::Inputs::EKey::KEY_N))
			EDITOR_EXEC(LoadEmptyScene());

		if (inputManager.IsKeyPressed(VisionWindowing::Inputs::EKey::KEY_S))
		{
			if (inputManager.GetKeyState(VisionWindowing::Inputs::EKey::KEY_LEFT_SHIFT) == VisionWindowing::Inputs::EKeyState::KEY_UP)
...
void VisionEditor::Panels::MenuBar::CreateFileMenu()
{
...
void VisionEditor::Panels::MenuBar::CreateBuildMenu()
{
...
void VisionEditor::Panels::MenuBar::CreateWindowMenu()
{
...
void VisionEditor::Panels::MenuBar::CreateSettingsMenu()
{
	...
	cameraSpeedMenu.CreateWidget<VisionUI::Widgets::Sliders::SliderInt>(1, 50, 15, VisionUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Scene View").ValueChangedEvent += EDITOR_BIND(SetSceneViewCameraSpeed, std::placeholders::_1);
	cameraSpeedMenu.CreateWidget<VisionUI::Widgets::Sliders::SliderInt>(1, 50, 15, VisionUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Asset View").ValueChangedEvent += EDITOR_BIND(SetAssetViewCameraSpeed, std::placeholders::_1);
	...
	auto& sceneViewBackgroundPicker = sceneViewBackground.CreateWidget<Selection::ColorEdit>(false, VisionUI::Types::Color{ 0.098f, 0.098f, 0.098f });
    ...
    auto& sceneViewGridPicker = sceneViewGrid.CreateWidget<Selection::ColorEdit>(false, VisionUI::Types::Color(0.176f, 0.176f, 0.176f));
	...
	{
		EDITOR_PANEL(Panels::SceneView, "Scene View").SetGridColor(VisionMaths::FVector3(0.176f, 0.176f, 0.176f));
		sceneViewGridPicker.color = VisionUI::Types::Color(0.176f, 0.176f, 0.176f);
	};

	auto& assetViewBackground = viewColors.CreateWidget<MenuList>("Asset View Background");
	auto& assetViewBackgroundPicker = assetViewBackground.CreateWidget<Selection::ColorEdit>(false, VisionUI::Types::Color{ 0.098f, 0.098f, 0.098f });
	...
	auto& assetViewGridPicker = assetViewGrid.CreateWidget<Selection::ColorEdit>(false, VisionUI::Types::Color(0.176f, 0.176f, 0.176f));
	...
	{
		EDITOR_PANEL(Panels::AssetView, "Asset View").SetGridColor(VisionMaths::FVector3(0.176f, 0.176f, 0.176f));
		assetViewGridPicker.color = VisionUI::Types::Color(0.176f, 0.176f, 0.176f);
	};

	auto& sceneViewBillboardScaleMenu = settingsMenu.CreateWidget<MenuList>("3D Icons Scales");
	auto& lightBillboardScaleSlider = sceneViewBillboardScaleMenu.CreateWidget<Sliders::SliderInt>(0, 100, static_cast<int>(Settings::EditorSettings::LightBillboardScale * 100.0f), VisionUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Lights");
...
void VisionEditor::Panels::MenuBar::CreateLayoutMenu() 
{
	auto& layoutMenu = CreateWidget<MenuList>("Layout");
	layoutMenu.CreateWidget<MenuItem>("Reset").ClickedEvent += EDITOR_BIND(ResetLayout);
}

void VisionEditor::Panels::MenuBar::CreateHelpMenu()
{
    auto& helpMenu = CreateWidget<MenuList>("Help");
    helpMenu.CreateWidget<MenuItem>("GitHub").ClickedEvent += [] {VisionTools::Utils::SystemCalls::OpenURL("https://github.com/darkvergus/Vision"); };
    helpMenu.CreateWidget<MenuItem>("Tutorials").ClickedEvent += [] {VisionTools::Utils::SystemCalls::OpenURL("https://github.com/darkvergus/Vision/wiki/Tutorials"); };
    helpMenu.CreateWidget<MenuItem>("Scripting API").ClickedEvent += [] {VisionTools::Utils::SystemCalls::OpenURL("https://github.com/darkvergus/Vision/wiki/Scripting-API"); };
    helpMenu.CreateWidget<Visual::Separator>();
    helpMenu.CreateWidget<MenuItem>("Bug Report").ClickedEvent += [] {VisionTools::Utils::SystemCalls::OpenURL("https://github.com/darkvergus/Vision/issues/new?assignees=&labels=Bug&template=bug_report.md&title="); };
    helpMenu.CreateWidget<MenuItem>("Feature Request").ClickedEvent += [] {VisionTools::Utils::SystemCalls::OpenURL("https://github.com/darkvergus/Vision/issues/new?assignees=&labels=Feature&template=feature_request.md&title="); };
    helpMenu.CreateWidget<Visual::Separator>();
    helpMenu.CreateWidget<Texts::Text>("Version: 1.0.0");
}

void VisionEditor::Panels::MenuBar::RegisterPanel(const std::string& p_name, VisionUI::Panels::PanelWindow& p_panel)
{
	auto& menuItem = m_windowMenu->CreateWidget<MenuItem>(p_name, "", true, true);
	menuItem.ValueChangedEvent += std::bind(&VisionUI::Panels::PanelWindow::SetOpened, &p_panel, std::placeholders::_1);

	m_panels.emplace(p_name, std::make_pair(std::ref(p_panel), std::ref(menuItem)));
}

void VisionEditor::Panels::MenuBar::UpdateToggleableItems()
{
	for (auto&[name, panel] : m_panels)
		panel.second.get().checked = panel.first.get().IsOpened();
}

void VisionEditor::Panels::MenuBar::OpenEveryWindows(bool p_state)
{
	for (auto&[name, panel] : m_panels)
