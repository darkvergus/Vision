...
namespace VisionEditor::Core
{
	...
	*/
	...
	public:
		...
		* @param p_canvas
		*/
		PanelsManager(VisionUI::Modules::Canvas& p_canvas);
		...
		*/
		...
		{
			if constexpr (std::is_base_of<VisionUI::Panels::PanelWindow, T>::value)
			{
				m_panels.emplace(p_id, std::make_unique<T>(p_id, std::forward<Args>(p_args)...));
				T& instance = *static_cast<T*>(m_panels.at(p_id).get());
				GetPanelAs<VisionEditor::Panels::MenuBar>("Menu Bar").RegisterPanel(instance.name, instance);
			}
			else
			{
	...
	private:
		std::unordered_map<std::string, std::unique_ptr<VisionUI::Panels::APanel>> m_panels;
