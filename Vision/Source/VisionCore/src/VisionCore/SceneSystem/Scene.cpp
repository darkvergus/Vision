...
VisionCore::SceneSystem::Scene::Scene()
{
...
VisionCore::SceneSystem::Scene::~Scene()
{
	std::for_each(m_actors.begin(), m_actors.end(), [](VisionCore::ECS::Actor* element)
	{ 
		delete element;
	});
...
void VisionCore::SceneSystem::Scene::Play()
{
	m_isPlaying = true;
...
bool VisionCore::SceneSystem::Scene::IsPlaying() const
{
	return m_isPlaying;
}

void VisionCore::SceneSystem::Scene::Update(float p_deltaTime)
{
...
void VisionCore::SceneSystem::Scene::FixedUpdate(float p_deltaTime)
{
	auto actors = m_actors;
	std::for_each(actors.begin(), actors.end(), std::bind(std::mem_fn(&ECS::Actor::OnFixedUpdate), std::placeholders::_1, p_deltaTime));
}

void VisionCore::SceneSystem::Scene::LateUpdate(float p_deltaTime)
{
	auto actors = m_actors;
	std::for_each(actors.begin(), actors.end(), std::bind(std::mem_fn(&ECS::Actor::OnLateUpdate), std::placeholders::_1, p_deltaTime));
}

VisionCore::ECS::Actor& VisionCore::SceneSystem::Scene::CreateActor()
{
	return CreateActor("New Actor");
}

VisionCore::ECS::Actor& VisionCore::SceneSystem::Scene::CreateActor(const std::string& p_name, const std::string& p_tag)
{
	m_actors.push_back(new VisionCore::ECS::Actor(m_availableID++, p_name, p_tag, m_isPlaying));
...
bool VisionCore::SceneSystem::Scene::DestroyActor(ECS::Actor& p_target)
{
	auto found = std::find_if(m_actors.begin(), m_actors.end(), [&p_target](VisionCore::ECS::Actor* element)
	{
...
void VisionCore::SceneSystem::Scene::CollectGarbages()
{
...
VisionCore::ECS::Actor* VisionCore::SceneSystem::Scene::FindActorByName(const std::string& p_name)
{
	auto result = std::find_if(m_actors.begin(), m_actors.end(), [p_name](VisionCore::ECS::Actor* element)
	{ 
...
VisionCore::ECS::Actor* VisionCore::SceneSystem::Scene::FindActorByTag(const std::string & p_tag)
{
	auto result = std::find_if(m_actors.begin(), m_actors.end(), [p_tag](VisionCore::ECS::Actor* element)
	{
...
VisionCore::ECS::Actor* VisionCore::SceneSystem::Scene::FindActorByID(int64_t p_id)
{
	auto result = std::find_if(m_actors.begin(), m_actors.end(), [p_id](VisionCore::ECS::Actor* element)
	{
...
std::vector<std::reference_wrapper<VisionCore::ECS::Actor>> VisionCore::SceneSystem::Scene::FindActorsByName(const std::string & p_name)
{
	std::vector<std::reference_wrapper<VisionCore::ECS::Actor>> actors;
...
std::vector<std::reference_wrapper<VisionCore::ECS::Actor>> VisionCore::SceneSystem::Scene::FindActorsByTag(const std::string & p_tag)
{
	std::vector<std::reference_wrapper<VisionCore::ECS::Actor>> actors;
...
void VisionCore::SceneSystem::Scene::OnComponentAdded(ECS::Components::AComponent& p_compononent)
{
...
void VisionCore::SceneSystem::Scene::OnComponentRemoved(ECS::Components::AComponent& p_compononent)
{
	if (auto result = dynamic_cast<ECS::Components::CModelRenderer*>(&p_compononent))
...
std::vector<VisionCore::ECS::Actor*>& VisionCore::SceneSystem::Scene::GetActors()
{
	return m_actors;
}

const VisionCore::SceneSystem::Scene::FastAccessComponents& VisionCore::SceneSystem::Scene::GetFastAccessComponents() const
{
	return m_fastAccessComponents;
}

void VisionCore::SceneSystem::Scene::OnSerialize(tinyxml2::XMLDocument & p_doc, tinyxml2::XMLNode * p_root)
{
...
void VisionCore::SceneSystem::Scene::OnDeserialize(tinyxml2::XMLDocument & p_doc, tinyxml2::XMLNode * p_root)
{
