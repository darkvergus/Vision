...
std::pair<VisionUI::Types::Color, std::string> GetWidgetSettingsFromLogData(const VisionDebug::LogData& p_logData)
{
	VisionUI::Types::Color logColor;
	...
	{
	default:
	case VisionDebug::ELogLevel::LOG_DEFAULT:	return { { 1.f, 1.f, 1.f, 1.f }, logDateFormated };
	case VisionDebug::ELogLevel::LOG_INFO:		return { { 0.f, 1.f, 1.f, 1.f }, logDateFormated };
	case VisionDebug::ELogLevel::LOG_WARNING:	return { { 1.f, 1.f, 0.f, 1.f }, logDateFormated };
	case VisionDebug::ELogLevel::LOG_ERROR:		return { { 1.f, 0.f, 0.f, 1.f }, logDateFormated };
	}
}

VisionEditor::Panels::Console::Console
(
	...
	PanelWindow(p_title, p_opened, p_windowSettings)
{
	...
	VisionDebug::Logger::LogEvent += std::bind(&Console::OnLogIntercepted, this, std::placeholders::_1);
}

void VisionEditor::Panels::Console::OnLogIntercepted(const VisionDebug::LogData & p_logData)
{
...
void VisionEditor::Panels::Console::ClearOnPlay()
{
	if (m_clearOnPlay)
		Clear();
}

void VisionEditor::Panels::Console::Clear()
{
	m_logTextWidgets.clear();
	m_logGroup->RemoveAllWidgets();
}

void VisionEditor::Panels::Console::FilterLogs()
{
	for (const auto&[widget, logLevel] : m_logTextWidgets)
		widget->enabled = IsAllowedByFilter(logLevel);
}

bool VisionEditor::Panels::Console::IsAllowedByFilter(VisionDebug::ELogLevel p_logLevel)
{
	switch (p_logLevel)
	{
	case VisionDebug::ELogLevel::LOG_DEFAULT:	return m_showDefaultLog;
	case VisionDebug::ELogLevel::LOG_INFO:		return m_showInfoLog;
	case VisionDebug::ELogLevel::LOG_WARNING:	return m_showWarningLog;
	case VisionDebug::ELogLevel::LOG_ERROR:		return m_showErrorLog;
	}

	return false;
}

void VisionEditor::Panels::Console::SetShowDefaultLogs(bool p_value)
{
	m_showDefaultLog = p_value;
	FilterLogs();
}

void VisionEditor::Panels::Console::SetShowInfoLogs(bool p_value)
{
	m_showInfoLog = p_value;
	FilterLogs();
}

void VisionEditor::Panels::Console::SetShowWarningLogs(bool p_value)
{
	m_showWarningLog = p_value;
	FilterLogs();
}

void VisionEditor::Panels::Console::SetShowErrorLogs(bool p_value)
{
