...
const VisionRendering::Geometry::BoundingSphere& VisionRendering::Resources::Model::GetBoundingSphere() const
{
	return m_boundingSphere;
}

VisionRendering::Resources::Model::Model(const std::string & p_path) : path(p_path)
{
}

VisionRendering::Resources::Model::~Model()
{
	for (auto mesh : m_meshes)
		delete mesh;
}

void VisionRendering::Resources::Model::ComputeBoundingSphere()
{
	if (m_meshes.size() == 1)
	{
	...
	else
	{
		...
		if (!m_meshes.empty())
		{
			...
			m_boundingSphere.radius = VisionMaths::FVector3::Distance(m_boundingSphere.position, { minX, minY, minZ });
		}
	}
}

const std::vector<VisionRendering::Resources::Mesh*>& VisionRendering::Resources::Model::GetMeshes() const
{
	return m_meshes;
}

const std::vector<std::string>& VisionRendering::Resources::Model::GetMaterialNames() const
{
